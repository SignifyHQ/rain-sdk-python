...
from rain_hello_world import RainHelloWorld, AsyncRainHelloWorld, DefaultAioHttpClient
from rain_hello_world._utils import is_dict
...
logging.getLogger("rain_hello_world").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[RainHelloWorld]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with RainHelloWorld(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncRainHelloWorld]:
